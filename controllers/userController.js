...
exports.signup = async (req, res) => {
    const schema = joi.object().keys({
        username: joi.string().required().error(e => "firstname is required"),
    ...
    var userdetails = new User({ username: req.body.username,  email: req.body.email,  accesstoken: accesstoken,  password: md5(req.body.password) })
...
exports.login = async (req, res) => {
    ...
    try {
            const schema = joi.object().keys({
            username: joi.string().required().error(e => "username is required"),
            password: joi.string().required().error(e => "Password is required")
        });
        ...
        var userdetails = await User.findOne({ username: req.body.username, password: md5(req.body.password) }).exec()
